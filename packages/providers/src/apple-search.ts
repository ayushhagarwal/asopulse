...
export type AppStoreProvider = {
  ...
  searchKeyword(term: string, country: string): Promise<StoreApp[]>;
};
...
export class AppleSearchProvider implements AppStoreProvider {
  ...
  constructor(
    ...
    private readonly cacheMs = 7 * 24 * 60 * 60 * 1000,
    private readonly minimumRequestGapMs = 3_100,
  ) {}

  async searchApps(term: string, country: string, limit = 25): Promise<StoreApp[]> {
    return this.request(term, country, Math.min(200, Math.max(1, limit)));
  }

  async searchKeyword(term: string, country: string): Promise<StoreApp[]> {
    return this.request(term, country, 200);
  }

  private async request(term: string, country: string, limit: number): Promise<StoreApp[]> {
